...
- **同会话多连接暂缓**：同一会话被多个连接同时打开时事件仍随机分发（P0/P1 遗留，协议 v2 注记"P2 处理广播/隔离"——已决策暂缓）。P2 前端为单标签页单连接，不受影响；多标签页同开才会踩到。
...
- **同会话多连接**：同一会话被多个连接同时打开时，事件会随机分发给其中一个连接（P2 决策：暂缓，见 P2 已知限制）。
